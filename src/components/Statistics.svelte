<script lang="ts">
    ...
    function requestTxData(): Promise<{data: string, value: number}[]> {
    ...
    let txData: {data: string, value: number}[] | null = null;

    onMount(() => {
        requestTxData().then(data => {
            txData = data;
            loading = false;
        })
    })
...
<!--    <div>тут будет график. Количество транзакций = {txData.length}</div>-->
{/if}
